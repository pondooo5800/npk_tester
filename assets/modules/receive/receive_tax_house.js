$(function () {
    ...
    $('#btn-del').on('click', function (e) {
        ...
        window.location.replace(domain + 'receive/' + 'receive_house_delete' + '/' + id);
    });
    ...
    var table = $('#tax_table').DataTable({
        ...
        lengthChange: false,
        ajax: {
            url: domain + 'receive/getAjaxReceived_tax_house',
        },
        "columnDefs": [{
            "name": "",
            "targets": 0
...

            {
                ...
                render: function (data, type, row) {
                    var btn =
                        ...
                        '<button type="button" onclick="window.location.href=\'' + domain + 'receive/receive_tax_pay_edit_house/' + '' + row['individual_id'] + '/' + row['tax_id'] + '/' + data + '\'" id="edit-notice" class="btn btn-warning btn-sm" title="แก้ไข">แก้ไข</button>' +
